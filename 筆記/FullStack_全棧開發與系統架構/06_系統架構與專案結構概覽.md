...
|前端 : RandomDog.vue|
|---|
使用者點擊「收藏這張」按鈕<br>@click="saveDog"

&emsp;&emsp; ⬇️

|HTTP POST 請求|
|---|
POST /api/dogs/<br>Body: { "url": "https://..." }
...
|資料庫 : SQLite|
|---|
INSERT INTO api_dogimage (url, created_at)<br>VALUES ('https://...', NOW())
...
|HTTP 回應|
|---|
Status: 201 Created<br>Body: { "id": 1, "url": "...", "created_at": "..." }

&emsp;&emsp; ⬇️

|前端 : 接收回應|
|---|
response.data → 包含新建的資料 ID<br>alert('收藏成功！')<br>可選：更新本地狀態或重新取得列表
